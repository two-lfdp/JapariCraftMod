...
public class LuckyHelper {
    ...
    static boolean isNotEmptyItemStack(ItemStack stack)
    {
        return (stack != null);
    }
    ...
    private static int getCanStoreSlot(IInventory inventory, ItemStack stack)
    {
        for (int slot = 0; slot < inventory.getSizeInventory(); ++slot)
        {
            ...
            if (isNotEmptyItemStack(stackInv))
            {
                ...
                boolean isStackSizeEqual = (stackInv.isStackable() && (stackInv.getMaxStackSize() < stackInv.getMaxStackSize()) && (stackInv.getMaxStackSize() < inventory.getInventoryStackLimit()));
